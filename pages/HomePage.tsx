...
// --- HAUS INSPIRED SCROLL NARRATIVE COMPONENT ---
const NarrativeScroll: React.FC = () => {
    ...
    const y1 = useTransform(scrollYProgress, [0, 0.1, 0.3], [100, 0, -100]);
    const scale1 = useTransform(scrollYProgress, [0, 0.15], [0.9, 1]);

    // 2nd Block: 30% to 65%
    const op2 = useTransform(scrollYProgress, [0.3, 0.4, 0.55, 0.65], [0, 1, 1, 0]);
    const y2 = useTransform(scrollYProgress, [0.3, 0.4, 0.65], [100, 0, -100]);
    const scale2 = useTransform(scrollYProgress, [0.3, 0.45], [0.9, 1]);

    // 3rd Block: 65% to 100%
    const op3 = useTransform(scrollYProgress, [0.65, 0.75, 0.9, 1], [0, 1, 1, 0]);
    const y3 = useTransform(scrollYProgress, [0.65, 0.75, 1], [100, 0, -50]);
    const scale3 = useTransform(scrollYProgress, [0.65, 0.8], [0.9, 1]);

    // Background Graphic scale/opacity
    const bgScale = useTransform(scrollYProgress, [0, 1], [1, 1.2]);
    const bgOpacity = useTransform(scrollYProgress, [0, 0.5, 1], [0.1, 0.3, 0.1]);

    return (
        <section ref={containerRef} className="relative h-[400vh] bg-cyber-bg border-t border-white/5">
            <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden">
                
                {/* Background Element morphing via scroll */}
                <motion.div 
                ...
                >
                    <img 
                        ...
                        alt="" 
                    />
                    <div className="absolute inset-0 bg-cyber-bg/80 backdrop-blur-md" />
                </motion.div>

                {/* Floating Tracker */}
                <div className="absolute top-1/2 left-8 md:left-12 -translate-y-1/2 z-20 flex flex-col gap-4 items-center">
                    <motion.div style={{ opacity: op1 }} className="w-1.5 h-1.5 bg-brand-purple rounded-full" />
                    <motion.div style={{ opacity: op2 }} className="w-1.5 h-1.5 bg-brand-yellow rounded-full" />
                    <motion.div style={{ opacity: op3 }} className="w-1.5 h-1.5 bg-emerald-500 rounded-full" />
                    <div className="w-px h-32 bg-white/10 mt-4 relative overflow-hidden">
                        <motion.div 
                ...
                <div className="relative z-10 container mx-auto px-16 md:px-32 max-w-7xl h-full flex items-center">
                    
                    <motion.div style={{ opacity: op1, y: y1, scale: scale1 }} className="absolute w-full max-w-5xl">
                        <span className="font-mono text-brand-purple uppercase tracking-[0.2em] text-xs font-bold mb-6 block">
                            // THE THESIS
                        </span>
                        <h2 className="text-4xl md:text-7xl lg:text-8xl font-bold tracking-tight text-white leading-[1.1]">
                            We don't chase temporary design cycles.
                        </h2>
                    </motion.div>

                    <motion.div style={{ opacity: op2, y: y2, scale: scale2 }} className="absolute w-full max-w-5xl">
                        <span className="font-mono text-brand-yellow uppercase tracking-[0.2em] text-xs font-bold mb-6 block">
                            // THE FRAMEWORK
                        </span>
                        <h2 className="text-4xl md:text-7xl lg:text-8xl font-bold tracking-tight text-white leading-[1.1]">
                            We execute strict architectures around functional logic.
                        </h2>
                    </motion.div>

                    <motion.div style={{ opacity: op3, y: y3, scale: scale3 }} className="absolute w-full max-w-5xl">
                        <span className="font-mono text-emerald-500 uppercase tracking-[0.2em] text-xs font-bold mb-6 block">
                            // THE OUTCOME
                        </span>
                        <h2 className="text-4xl md:text-7xl lg:text-8xl font-bold tracking-tight text-white leading-[1.1] mb-12">
                            Filtering subjective feedback into structural business equity.
                        </h2>
                        <Link to="/about" className="inline-flex items-center gap-3 font-mono text-xs uppercase tracking-widest text-white border border-white/10 hover:border-emerald-500 px-8 py-4 rounded-full bg-white/[0.02] transition-colors duration-300">
                            Verify Architecture Details <DownArrow className="rotate-[-90deg] text-emerald-500" size={14} />
                        </Link>
                    </motion.div>
